"""Fare estimation API server using FastAPI."""
...
@app.get("/health")
def health():
    return {"success": True, "message": "Python fare server is running"}


@app.get("/api/v1/fare/estimate")
def get_fare_estimate():
    return {"success": True, "message": "Python fare estimate GET endpoint is working"}
...
def estimate_fare(data: dict):
    ...
    result = calculate_fare(
        ...
        two_way_discount_percent=float(data["two_way_discount_percent"]),
    )
